...
final class EmptySectionRule implements RuleInterface
{
    ...
    public function analyze(string $file, string $source, ParsingState $parsingState, Configuration $configuration): array
    {
        ...
        $walker->walk(
            ...
            function (ViewHelperNode $node) use (&$issues, $walker, $file, $source): void {
                if (!$walker->isCoreViewHelper($node, 'section')) {
                    return;
                }

                if (!$walker->sectionIsEmpty($node)) {
                    return;
                }

                $name = $this->argumentExtractor->scalarArgument($node, 'name') ?? 'unknown';
                $issues[] = new Issue(
                    ruleId: $this->getId(),
                    severity: Severity::Warning,
                    message: sprintf('Section "%s" has no content.', $name),
                    file: $file,
